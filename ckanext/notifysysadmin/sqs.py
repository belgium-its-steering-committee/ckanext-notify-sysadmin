...
def send_sqs_message(user, subject, message):
    ...
    #messagesAttributes
    #TODO make sender_email as config entry
    message_attributes = { 
            'sender_email':{
    ...
    if (user.email is None) or not len(user.email):
        ...
        if "ResponseMetadata" in response:
            ...
            if statusCode == 200:
                ...
                log.warn("New organization creation message sending returned statuscode {0}".format(str( statusCode)))
                raise Exception("Failed to send new organization creation message. Statuscode {0}".format(str(statusCode)))
        else:
